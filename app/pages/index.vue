<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-200">
    <div>
      <MenuBar />
      <div
        class="w-md h-170 text-center m-auto relative bg-gray-400 rounded-md shadow-2xl px-5 mt-5 overflow-y-auto scrollbar-hide"
      >
        <div
          v-if="dialog.open"
              ...
              <div class="flex items-center justify-center ml-2">
                ...
                <label :for="'check' + item.id" class="cursor-pointer">
                  <del v-if="item.done" class="text-xl ml-3 text-gray-300"
                    >《{{ item.text }}》</del
                  >
                  <span v-else class="text-xl ml-3 text-black"
                    >《{{ item.text }}》</span
                  >
                </label>
              </div>
